...
(function($) {
    ...
    window.ShopGlutAccordionLayout = window.ShopGlutAccordionLayout || {};

    // Data converter module namespace
    ShopGlutAccordionLayout.dataConverter = {
        ...
        shouldSkipField: function(name) {
            ...
            // Skip non-Accordion layout related fields
            var skipFields = [
                'agl_metabox_nonce',
                '_wp_http_referer',
                '_pseudo',
                'shopg_Accordion_layouts_nonce',
                'shopg_shop_layoutid',
                'layout_name',
                'layout_template'
            ];
         ...
         * e.g., "shopg_Accordion_settings_template1[tab][field]" -> {shopg_Accordion_settings_template1: {tab: {field: value}}}
         */
        ...
        /**
         * Parse a field name like "shopg_Accordion_settings_template1[tab][field][0]" into an array of keys
         */
         ...
         */
        saveAccordionLayoutData: function() {
            // Show loader
            ...
            var nonce = $('input[name="shopg_Accordion_layouts_nonce"]').val();

            // Prepare AJAX data
            var ajaxData = {
                action: 'save_shopg_Accordion_layoutdata',
                layout_name: layoutName,
                layout_template: layoutTemplate,
                shopg_Accordion_layoutid: layoutId,
                Accordion_nonce: nonce,
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        ShopGlutAccordionLayout.dataConverter.showNotification('success', response.data.message || 'Order complete layout saved successfully!');
                        ...
                        // Show error message
                        ShopGlutAccordionLayout.dataConverter.showNotification('error', response.data.message || response.data || 'Failed to save order complete layout.');
                    }
                },
                error: function(xhr) {
                    ...
                    ShopGlutAccordionLayout.dataConverter.showNotification('error', errorMessage);
                },
                complete: function() {
                    // Hide loader
         ...
         */
        resetAccordionLayoutSettings: function() {
            ...
            // Prepare AJAX data
            var ajaxData = {
                action: 'reset_shopg_Accordion_layout_settings',
                layout_id: layoutId,
                nonce: nonce
            };
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // Show success message
                        ShopGlutAccordionLayout.dataConverter.showNotification('success', response.data.message || 'Settings reset to default successfully!');
                        ...
                        // Show error message
                        ShopGlutAccordionLayout.dataConverter.showNotification('error', response.data.message || response.data || 'Failed to reset settings.');
                    }
                },
                error: function() {
                    ShopGlutAccordionLayout.dataConverter.showNotification('error', 'Network error occurred while resetting settings.');
                },
                complete: function() {
                    // Hide loader
                    if (window.hideLoader && typeof window.hideLoader === 'function') {
    ...
    $(document).ready(function() {
        ...
        $('#Accordion-save-layout-button').on('click', function(e) {
            e.preventDefault();
            ShopGlutAccordionLayout.dataConverter.saveAccordionLayoutData();
        });

        // Bind reset button click
        $('#Accordion-reset-settings-button').on('click', function(e) {
            e.preventDefault();
            ShopGlutAccordionLayout.dataConverter.resetAccordionLayoutSettings();
        });

        // Accordion display selection AJAX - following select_products pattern
        $(".agl-field.agl-field-select_accordion_display .agl-fieldset").on("click", function () {
            if ($(".agl-field.agl-field-select_accordion_display .chosen-drop .chosen-results").length > 0) {

                // Validate required data exists
                if (!shopglut_admin_vars || !shopglut_admin_vars.ajax_url) {
                    console.warn('Missing required data for accordion display options request');
                    return;
                }
                ...
                if (!nonce) {
                    console.warn('Missing nonce for accordion display options request');
                    return;
                }

                // Show loading spinner
                $(".agl-field.agl-field-select_accordion_display .chosen-drop").append('<div class="loading-spinner"></div>');

                $.ajax({
                    url: shopglut_admin_vars.ajax_url || ajaxurl,
                    method: "POST",
                    data: {
                        action: "shopglut_get_accordion_display_options",
                        nonce: nonce,
                        layout_id: layoutId,
                    },
                    timeout: 15000, // 15 second timeout
                    success: function(response) {
                        console.log('Accordion display options response:', response);
                        ...
                        if (response.success && response.data) {
                            ...
                            $(".agl-field.agl-field-select_accordion_display .chosen-drop .chosen-results li").each(function() {
                                var itemText = $(this).text().trim();
                                // Remove the " (Used by another layout)" suffix for accordion
                    ...
                    error: function (jqXHR, textStatus, errorThrown) {
                        console.error("Failed to retrieve accordion display options:", {
                            status: textStatus,
                            error: errorThrown,
                        ...
                        if (jqXHR.status === 403) {
                            ShopGlutAccordionLayout.dataConverter.showNotification('error', 'Access denied. Please refresh the page.');
                        }
                    },
                    complete: function () {
                        // Remove loading spinner
                        $(".agl-field.agl-field-select_accordion_display .chosen-drop .loading-spinner").remove();
                        $(".agl-field.agl-field-select_accordion_display .chosen-results").removeClass("filter-dis");
                    }
                });
            }
        });
