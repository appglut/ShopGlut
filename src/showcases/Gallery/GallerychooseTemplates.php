...
class GallerychooseTemplates {

	public function __construct() {
		add_action('admin_post_create_gallery_layout', array($this, 'handleCreateGalleryEnhancement'));
		add_action('wp_ajax_get_gallery_demo_content', array($this, 'handleGetGalleryDemoContent'));
	}

	public function loadGalleryTemplates() {
		$tab_names = [
			'tab1' => [
						...
						<div class="shopg-template-preview" data-template="<?php echo esc_attr($layout_template); ?>">
							...
							<div class="template-content product-demo">
								...
								<div class="template-html-container">
									...
									<div class="html-preview-background">
										<?php $this->renderGalleryPreview($layout_template); ?>
									</div>

									<!-- Foreground: View Demo Button with Overlay -->
									<div class="html-preview-overlay">
										<button type="button" class="demo-view-btn" onclick="openHtmlDemoModal('<?php echo esc_attr($layout_template); ?>', 'product-gallery')">
							...
							<div class="template-footer">
								<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
									<input type="hidden" name="action" value="create_gallery_layout">
									<input type="hidden" name="layout_template" value="<?php echo esc_attr($layout_template); ?>">
									<?php wp_nonce_field('create_gallery_layout_nonce', 'create_gallery_layout_nonce'); ?>
		...
		.shopg-template-gallery .html-preview-background .gallery-modal-content,
		.shopg-template-gallery .html-preview-background .shopglut-product-gallery .gallery-modal-content {
		...
		/* Override gallery modal styles in preview */
		.shopg-template-gallery .html-preview-background .gallery-modal {
			position: static !important;
		...
		/* Ensure the gallery preview container is centered and fits properly */
		.shopg-template-gallery .html-preview-background .shopglut-gallery-preview-container {
			position: absolute !important;
			display: flex !important;
			align-items: center !important;
			justify-content: center !important;
		...
		.shopg-template-gallery .html-preview-background .gallery-modal,
		.shopg-template-gallery .html-preview-background .gallery-modal-content,
		.shopg-template-gallery .html-preview-background .gallery-inner {
			overflow: hidden !important;
			max-height: none !important;
		...
		.shopg-template-gallery .html-preview-background .shopglut-product-gallery {
			overflow: hidden !important;
		...
		.shopg-template-gallery .html-preview-background .gallery-gallery,
		.shopg-template-gallery .html-preview-background .gallery-info {
			overflow: hidden !important;
			max-height: none !important;
		}
		...
		.shopg-template-gallery .html-preview-background .gallery-modal-overlay {
			display: none !important;
		}
		...
		.shopg-template-gallery .html-preview-background .gallery-close {
			display: none !important;
		}

		/* Fix Live Demo modal visibility issue - override template1 style conflicts */
		#htmlDemoModal .shopglut-product-gallery.template1 .gallery-modal {
			opacity: 1 !important;
			visibility: visible !important;
			position: fixed !important;
			display: flex !important;
		...
		function testGalleryAjax() {
			fetch(ajax_url, {
				...
				body: new URLSearchParams({
					action: 'get_gallery_demo_content',
		...
		// Test on page load
		setTimeout(testGalleryAjax, 2000);
				...
				if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
					mutation.addedNodes.forEach(function(node) {
						if (node.nodeType === 1) { // Element node
							...
							if (node.classList && node.classList.contains('shopglut-product-gallery') ||
								(node.id && node.id === 'htmlDemoModal') ||
								(node.querySelector && node.querySelector('.shopglut-product-gallery'))) {
		...
		// Keep image modal functionality for gallery images
		function openImageModal(imageName) {
			const modal = document.getElementById('imageModal');
			const mainImage = document.getElementById('modalMainImage');
			const assetsUrl = '<?php echo esc_js(SHOPGLUT_URL . "global-assets/images/gallery-templates/"); ?>';
	 ...
	 */
	private function renderGalleryPreview($layout_template) {
		// Include the template markup class
		require_once SHOPGLUT_PATH . 'src/showcases/Gallery/templates/template1/template1Markup.php';
		require_once SHOPGLUT_PATH . 'src/showcases/Gallery/templates/template1/template1Style.php';

		$markup = new \Shopglut\showcases\Gallery\templates\template1\template1Markup();
		$style = new \Shopglut\showcases\Gallery\templates\template1\template1Style();

		// Render styles inline for preview
		$style->dynamicCss(0);
		?>
		<div class="shopglut-gallery-preview-container">
			<?php
			// Render the demo content directly from template1Markup
			// Get default settings for demo
			$settings = $this->getDefaultGalleryettings();
			$markup->render_demo_gallery($settings, []);
			?>
		</div>
		<?php
	}
	 ...
	 */
	private function getDefaultGalleryettings() {
		return array(
			'modal_width' => '1100px',
	 ...
	 */
	private function renderGalleryFullDemo($layout_template) {
		// Include the template markup class
		require_once SHOPGLUT_PATH . 'src/showcases/Gallery/templates/template1/template1Markup.php';
		require_once SHOPGLUT_PATH . 'src/showcases/Gallery/templates/template1/template1Style.php';
		...
		// Render the full demo gallery
		$markup->render_demo_gallery($settings, []);
	}

	/**
	 * Handle AJAX request for gallery demo content
	 */
	public function handleGetGalleryDemoContent() {
		// Verify nonce
		...
		$this->renderGalleryFullDemo($template_id);
		wp_die();
	}

	public function handleCreateGalleryEnhancement() {
		if (
			!isset($_POST['create_gallery_layout_nonce']) ||
			!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['create_gallery_layout_nonce'])), 'create_gallery_layout_nonce') ||
	...
	// Use $wpdb->insert() with proper error handling and caching
			...
			$cache_key = 'shopglut_gallery_layouts';
			...
			$update_result = $wpdb->query(
				$wpdb->prepare(
					"UPDATE %s SET layout_name = %s WHERE id = %d", // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.UnsupportedIdentifierPlaceholder -- Using %s instead of %i for compatibility, table name escaped with esc_sql()
					$table_name,
