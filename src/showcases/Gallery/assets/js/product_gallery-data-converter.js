...
(function($) {
    ...
    window.ShopGlutGalleryLayout = window.ShopGlutGalleryLayout || {};

    // Data converter module namespace
    ShopGlutGalleryLayout.dataConverter = {
        ...
        shouldSkipField: function(name) {
            ...
            // Skip non-Gallery layout related fields
            var skipFields = [
                'agl_metabox_nonce',
                '_wp_http_referer',
                '_pseudo',
                'shopg_Gallery_layouts_nonce',
                'shopg_shop_layoutid',
                'layout_name',
                'layout_template'
            ];
         ...
         * e.g., "shopg_Gallery_settings_template1[tab][field]" -> {shopg_Gallery_settings_template1: {tab: {field: value}}}
         */
        ...
        /**
         * Parse a field name like "shopg_Gallery_settings_template1[tab][field][0]" into an array of keys
         */
         ...
         */
        saveGalleryLayoutData: function() {
            // Show loader
            ...
            var nonce = $('input[name="shopg_Gallery_layouts_nonce"]').val();

            // Prepare AJAX data
            var ajaxData = {
                action: 'save_shopg_Gallery_layoutdata',
                layout_name: layoutName,
                layout_template: layoutTemplate,
                shopg_Gallery_layoutid: layoutId,
                Gallery_nonce: nonce,
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        ShopGlutGalleryLayout.dataConverter.showNotification('success', response.data.message || 'Order complete layout saved successfully!');
                        ...
                        // Show error message
                        ShopGlutGalleryLayout.dataConverter.showNotification('error', response.data.message || response.data || 'Failed to save order complete layout.');
                    }
                },
                error: function(xhr) {
                    ...
                    ShopGlutGalleryLayout.dataConverter.showNotification('error', errorMessage);
                },
                complete: function() {
                    // Hide loader
         ...
         */
        resetGalleryLayoutSettings: function() {
            ...
            // Prepare AJAX data
            var ajaxData = {
                action: 'reset_shopg_Gallery_layout_settings',
                layout_id: layoutId,
                nonce: nonce
            };
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // Show success message
                        ShopGlutGalleryLayout.dataConverter.showNotification('success', response.data.message || 'Settings reset to default successfully!');
                        ...
                        // Show error message
                        ShopGlutGalleryLayout.dataConverter.showNotification('error', response.data.message || response.data || 'Failed to reset settings.');
                    }
                },
                error: function() {
                    ShopGlutGalleryLayout.dataConverter.showNotification('error', 'Network error occurred while resetting settings.');
                },
                complete: function() {
                    // Hide loader
                    if (window.hideLoader && typeof window.hideLoader === 'function') {
    ...
    $(document).ready(function() {
        ...
        $('#Gallery-save-layout-button').on('click', function(e) {
            e.preventDefault();
            ShopGlutGalleryLayout.dataConverter.saveGalleryLayoutData();
        });

        // Bind reset button click
        $('#Gallery-reset-settings-button').on('click', function(e) {
            e.preventDefault();
            ShopGlutGalleryLayout.dataConverter.resetGalleryLayoutSettings();
        });

        // Gallery display selection AJAX - following select_products pattern
        $(".agl-field.agl-field-select_gallery_display .agl-fieldset").on("click", function () {
            if ($(".agl-field.agl-field-select_gallery_display .chosen-drop .chosen-results").length > 0) {

                // Validate required data exists
                if (!shopglut_admin_vars || !shopglut_admin_vars.ajax_url) {
                    console.warn('Missing required data for gallery display options request');
                    return;
                }
                ...
                if (!nonce) {
                    console.warn('Missing nonce for gallery display options request');
                    return;
                }

                // Show loading spinner
                $(".agl-field.agl-field-select_gallery_display .chosen-drop").append('<div class="loading-spinner"></div>');

                $.ajax({
                    url: shopglut_admin_vars.ajax_url || ajaxurl,
                    method: "POST",
                    data: {
                        action: "shopglut_get_gallery_display_options",
                        nonce: nonce,
                        layout_id: layoutId,
                    },
                    timeout: 15000, // 15 second timeout
                    success: function(response) {
                        console.log('Gallery display options response:', response);
                        ...
                        if (response.success && response.data) {
                            ...
                            $(".agl-field.agl-field-select_gallery_display .chosen-drop .chosen-results li").each(function() {
                                var itemText = $(this).text().trim();
                                // Remove the " (Used by another layout)" suffix for gallery
                    ...
                    error: function (jqXHR, textStatus, errorThrown) {
                        console.error("Failed to retrieve gallery display options:", {
                            status: textStatus,
                            error: errorThrown,
                        ...
                        if (jqXHR.status === 403) {
                            ShopGlutGalleryLayout.dataConverter.showNotification('error', 'Access denied. Please refresh the page.');
                        }
                    },
                    complete: function () {
                        // Remove loading spinner
                        $(".agl-field.agl-field-select_gallery_display .chosen-drop .loading-spinner").remove();
                        $(".agl-field.agl-field-select_gallery_display .chosen-results").removeClass("filter-dis");
                    }
                });
            }
        });
