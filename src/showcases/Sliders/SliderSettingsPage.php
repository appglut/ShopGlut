...
class SliderSettingsPage {

	public $menu_slug = 'product_slider';

	public function __construct() {

	}

	public function loadSliderEditor() {
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
		$layout_data = $wpdb->get_row( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Direct query required for custom table operation
			$wpdb->prepare( "SELECT layout_name, layout_template FROM %i WHERE id = %d", $table_name, $layout_id ) ); // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.UnsupportedIdentifierPlaceholder -- WP 6.2+ identifier placeholder supported
	    ...
	    <form id="shopglut_shop_layouts" method="post" action="">
				...
				<?php 
				$shopg_cpage_nonce = wp_create_nonce( 'shopg_Slider_layouts' ); 
				?>
				<input type="hidden" name="shopg_Slider_layouts_nonce" value="<?php echo esc_attr( $shopg_cpage_nonce ); ?>">
			   ...
			   <div id="post-body" class="metabox-holder columns-2">
						...
						<div id="shopg-Slider-layout-settings" class="postbox-container shopg-layout-settings-wrapper">
						...
						<div class="submitbox" id="submitpost">
								<div id="SliderLayout-publishing-action" class="shopg-publishing-button">
									<button type="button" id="Slider-reset-settings-button" class="btn btn-fullwidth btn-secondary"
									style=" background: #dc3545; color: white; border: none;">
									<?php echo esc_attr__( 'Reset', 'shopglut' ); ?>
									</button>
									<button type="button" name="publish" id="Slider-save-layout-button" class="btn btn-fullwidth">
						...
						<div id="shopg-Slider-layout-container" class="shopg-admin-edit-panel shopg-layout-container">
		...
		<style>
			...
			/* When loader is visible, prevent interaction with slider demo */
			.loader-overlay:not([style*="display: none"]) ~ * .shopglut-product-slider {
