...
(function($) {
    ...
    window.ShopGlutSliderLayout = window.ShopGlutSliderLayout || {};

    // Data converter module namespace
    ShopGlutSliderLayout.dataConverter = {
        ...
        shouldSkipField: function(name) {
            ...
            // Skip non-Slider layout related fields
            var skipFields = [
                'agl_metabox_nonce',
                '_wp_http_referer',
                '_pseudo',
                'shopg_Slider_layouts_nonce',
                'shopg_shop_layoutid',
                'layout_name',
                'layout_template'
            ];
         ...
         * e.g., "shopg_Slider_settings_template1[tab][field]" -> {shopg_Slider_settings_template1: {tab: {field: value}}}
         */
        ...
        /**
         * Parse a field name like "shopg_Slider_settings_template1[tab][field][0]" into an array of keys
         */
        ...
        saveSliderLayoutData: function() {
            ...
            var nonce = $('input[name="shopg_Slider_layouts_nonce"]').val();

            // Prepare AJAX data
            var ajaxData = {
                action: 'save_shopg_Slider_layoutdata',
                layout_name: layoutName,
                layout_template: layoutTemplate,
                shopg_Slider_layoutid: layoutId,
                Slider_nonce: nonce,
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        ShopGlutSliderLayout.dataConverter.showNotification('success', response.data.message || 'Order complete layout saved successfully!');
                        ...
                        // Show error message
                        ShopGlutSliderLayout.dataConverter.showNotification('error', response.data.message || response.data || 'Failed to save order complete layout.');
                    }
                },
                error: function(xhr) {
                    ...
                    ShopGlutSliderLayout.dataConverter.showNotification('error', errorMessage);
                },
                complete: function() {
                    // Hide loader
         ...
         */
        resetSliderLayoutSettings: function() {
            ...
            // Prepare AJAX data
            var ajaxData = {
                action: 'reset_shopg_Slider_layout_settings',
                layout_id: layoutId,
                nonce: nonce
            };
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // Show success message
                        ShopGlutSliderLayout.dataConverter.showNotification('success', response.data.message || 'Settings reset to default successfully!');
                        ...
                        // Show error message
                        ShopGlutSliderLayout.dataConverter.showNotification('error', response.data.message || response.data || 'Failed to reset settings.');
                    }
                },
                error: function() {
                    ShopGlutSliderLayout.dataConverter.showNotification('error', 'Network error occurred while resetting settings.');
                },
                complete: function() {
                    // Hide loader
                    if (window.hideLoader && typeof window.hideLoader === 'function') {
    ...
    $(document).ready(function() {
        ...
        $('#Slider-save-layout-button').on('click', function(e) {
            e.preventDefault();
            ShopGlutSliderLayout.dataConverter.saveSliderLayoutData();
        });

        // Bind reset button click
        $('#Slider-reset-settings-button').on('click', function(e) {
            e.preventDefault();
            ShopGlutSliderLayout.dataConverter.resetSliderLayoutSettings();
        });

        // Slider display selection AJAX - following select_products pattern
        $(".agl-field.agl-field-select_slider_display .agl-fieldset").on("click", function () {
            if ($(".agl-field.agl-field-select_slider_display .chosen-drop .chosen-results").length > 0) {

                // Validate required data exists
                if (!shopglut_admin_vars || !shopglut_admin_vars.ajax_url) {
                    console.warn('Missing required data for slider display options request');
                    return;
                }
                ...
                if (!nonce) {
                    console.warn('Missing nonce for slider display options request');
                    return;
                }

                // Show loading spinner
                $(".agl-field.agl-field-select_slider_display .chosen-drop").append('<div class="loading-spinner"></div>');

                $.ajax({
                    url: shopglut_admin_vars.ajax_url || ajaxurl,
                    method: "POST",
                    data: {
                        action: "shopglut_get_slider_display_options",
                        nonce: nonce,
                        layout_id: layoutId,
                    },
                    timeout: 15000, // 15 second timeout
                    success: function(response) {
                        console.log('Slider display options response:', response);
                        ...
                        if (response.success && response.data) {
                            ...
                            $(".agl-field.agl-field-select_slider_display .chosen-drop .chosen-results li").each(function() {
                                var itemText = $(this).text().trim();
                                // Remove the " (Used by another layout)" suffix for slider
                    ...
                    error: function (jqXHR, textStatus, errorThrown) {
                        console.error("Failed to retrieve slider display options:", {
                            status: textStatus,
                            error: errorThrown,
                        ...
                        if (jqXHR.status === 403) {
                            ShopGlutSliderLayout.dataConverter.showNotification('error', 'Access denied. Please refresh the page.');
                        }
                    },
                    complete: function () {
                        // Remove loading spinner
                        $(".agl-field.agl-field-select_slider_display .chosen-drop .loading-spinner").remove();
                        $(".agl-field.agl-field-select_slider_display .chosen-results").removeClass("filter-dis");
                    }
                });
            }
        });
