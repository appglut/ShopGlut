...
(function($) {
    ...
    window.ShopGlutAcfPageLayout = window.ShopGlutAcfPageLayout || {};

    // Data converter module namespace
    ShopGlutAcfPageLayout.dataConverter = {
        ...
        shouldSkipField: function(name) {
            ...
            // Skip non-acfpage layout related fields
            var skipFields = [
                'agl_metabox_nonce',
                '_wp_http_referer',
                '_pseudo'
         ...
         * e.g., "shopg_acfpage_settings_template1[tab][field]" -> {shopg_acfpage_settings_template1: {tab: {field: value}}}
         */
        ...
        /**
         * Parse a field name like "shopg_acfpage_settings_template1[tab][field][0]" into an array of keys
         */
         ...
         */
        saveAcfPageLayoutData: function() {
            // Show loader
            ...
            var nonce = $('input[name="shopg_product_custom_field_layouts_nonce"]').val();

            // Prepare AJAX data
            var ajaxData = {
                action: 'save_shopg_product_custom_field_layoutdata',
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        ShopGlutAcfPageLayout.dataConverter.showNotification('success', response.data.message || 'Order complete layout saved successfully!');
                        ...
                        // Show error message
                        ShopGlutAcfPageLayout.dataConverter.showNotification('error', response.data.message || response.data || 'Failed to save order complete layout.');
                    }
                },
                error: function(xhr) {
                    ...
                    ShopGlutAcfPageLayout.dataConverter.showNotification('error', errorMessage);
                },
                complete: function() {
                    // Hide loader
         ...
         */
        resetAcfPageLayoutSettings: function() {
            ...
            // Prepare AJAX data
            var ajaxData = {
                action: 'reset_shopg_product_custom_field_settings',
                layout_id: layoutId,
                nonce: nonce
            };
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // Show success message
                        ShopGlutAcfPageLayout.dataConverter.showNotification('success', response.data.message || 'Settings reset to default successfully!');
                        ...
                        // Show error message
                        ShopGlutAcfPageLayout.dataConverter.showNotification('error', response.data.message || response.data || 'Failed to reset settings.');
                    }
                },
                error: function() {
                    ShopGlutAcfPageLayout.dataConverter.showNotification('error', 'Network error occurred while resetting settings.');
                },
                complete: function() {
                    // Hide loader
                    if (window.hideLoader && typeof window.hideLoader === 'function') {
    ...
    $(document).ready(function() {
        ...
        $('#product_custom_fieldLayout-publishing-action #publish').on('click', function(e) {
            e.preventDefault();
            ShopGlutAcfPageLayout.dataConverter.saveAcfPageLayoutData();
        });

        // Bind reset button click
        $('product_custom_fieldLayout-publishing-action #product_custom_field-reset-settings-button').on('click', function(e) {
            e.preventDefault();
            ShopGlutAcfPageLayout.dataConverter.resetAcfPageLayoutSettings();
        });
    });
