...
class CategoryShortcode {
	...
	public function render_category_shortcode($atts) {
		...
		if (!function_exists('wc_get_product')) {
			return '<p class="shopglut-error">' . esc_html__('WooCommerce is required for this shortcode to work.', 'shopglut') . '</p>';
		}
		...
		if (empty($atts['id'])) {
			return '<p class="shopglut-error">' . esc_html__('Category ID/slug is required for [shopglut_woo_category] shortcode.', 'shopglut') . '</p>';
		}
		...
		if (empty($categories)) {
			return '<p class="shopglut-error">' . esc_html__('No categories found.', 'shopglut') . '</p>';
		}
	...
	private function enqueue_assets($atts) {
		// Enqueue Bootstrap-like styles if not already loaded
		wp_enqueue_style('shopglut-category-shortcode', SHOPGLUT_URL . 'src/tools/shortcodeShowcase/assets/css/category-shortcode.css', array(), SHOPGLUT_VERSION);

		if ($atts['async']) {
			wp_enqueue_script('shopglut-category-shortcode', SHOPGLUT_URL . 'src/tools/shortcodeShowcase/assets/js/category-shortcode.js', array('jquery'), SHOPGLUT_VERSION, true);
			wp_localize_script('shopglut-category-shortcode', 'shopglutWooCategoryAjax', array(
	 ...
	 *
	 * @param array $values Category slugs or IDs
	 * @param string $field Field type (slug or id)
	 ...
	 * @param array $atts Shortcode attributes
	 * @return array Category data for display
	 */
	 ...
	 * @param array $atts Shortcode attributes
	 * @param array $category_data Category display data
	 ...
	 * @param string $content_id Content container ID
	 * @param array $atts Shortcode attributes
	 * @param array $category_data Category display data
	 */
	private function render_toolbar($form_id, $content_id, $atts, $category_data) {
		...
		echo '<input type="text" name="skw" value="' . esc_attr(isset($_GET['skw']) ? sanitize_text_field(wp_unslash($_GET['skw'])) : '') . '" placeholder="' . esc_attr__('Search Keyword...', 'shopglut') . '" class="form-control">';
		echo '</div>';

		// Order By dropdown
		echo '<div class="col-lg-' . esc_attr($grid[1]) . ' col-md-4">';
		echo '<select name="orderby" class="shopglut-custom-select custom-select">';
		echo '<option value="date" disabled="disabled">' . esc_html__('Order By:', 'shopglut') . '</option>';
		echo '<option value="date"' . selected($current_orderby, 'date', false) . '>' . esc_html__('Publish Date', 'shopglut') . '</option>';
		echo '<option value="title"' . selected($current_orderby, 'title', false) . '>' . esc_html__('Title', 'shopglut') . '</option>';
		echo '<option value="modified"' . selected($current_orderby, 'modified', false) . '>' . esc_html__('Update Date', 'shopglut') . '</option>';
		echo '<option value="total_sales"' . selected($current_orderby, 'total_sales', false) . '>' . esc_html__('Sales', 'shopglut') . '</option>';
		echo '<option value="menu_order"' . selected($current_orderby, 'menu_order', false) . '>' . esc_html__('Menu Order', 'shopglut') . '</option>';
		echo '<option value="price"' . selected($current_orderby, 'price', false) . '>' . esc_html__('Price', 'shopglut') . '</option>';
		echo '</select>';
		echo '</div>';
		...
		echo '<option value="desc" disabled="disabled">' . esc_html__('Order:', 'shopglut') . '</option>';
		echo '<option value="DESC"' . selected($current_order, 'DESC', false) . '>' . esc_html__('Descending', 'shopglut') . '</option>';
		echo '<option value="ASC"' . selected($current_order, 'ASC', false) . '>' . esc_html__('Ascending', 'shopglut') . '</option>';
		echo '</select>';
		echo '</div>';

		// Apply button
		echo '<div class="col-lg-' . esc_attr($grid[3]) . ' col-md-4">';
		echo '<button type="submit" class="btn btn-secondary btn-block">' . esc_html__('Apply Filter', 'shopglut') . '</button>';
		echo '</div>';
	 ...
	 *
	 * @param string $html Template HTML with tags
	 * @param WC_Product $product Product object
	...
	private function render_default_product($product) {
		...
		// Add to cart button
		echo '<div class="ml-3">';
		echo '<a href="' . esc_url($product->add_to_cart_url()) . '" class="btn btn-primary shopglut-add-to-cart">' . esc_html__('Add to Cart', 'shopglut') . '</a>';
		echo '</div>';
	...
	public function ajax_load_products() {
		...
		if (empty($categories)) {
			wp_send_json_error(array('message' => __('No categories found.', 'shopglut')));
		}
