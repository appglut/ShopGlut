<?php
namespace Shopglut\tools\shortcodeShowcase;
...
class ShortcodesShowcaseEntity {

	protected static function getTable() {
		return ShopGlutDatabase::table_shortcodes_showcase();
	}
	...
	public static function retrieveAll( $limit = 0, $current_page = 1 ) {
		...
		$cache_key = 'shopglut_shortcodes_all_' . md5( $limit . '_' . $current_page );
	...
	public static function retrieveAllCount() {
		...
		$cache_key = 'shopglut_shortcodes_count';
	...
	public static function delete_layout( $template_id ) {
		global $wpdb;
		...
		$result = $wpdb->delete( $table, [ 'id' => absint( $template_id ) ], [ '%d' ] );

		// Clear related cache after deletion
		if ( $result ) {
			wp_cache_delete( 'shopglut_shortcodes_count' );
