...
(function($) {
    ...
    window.ShopGlutProductQuickviewLayout = window.ShopGlutProductQuickviewLayout || {};

    // Data converter module namespace
    ShopGlutProductQuickviewLayout.dataConverter = {
        ...
        shouldSkipField: function(name) {
            ...
            // Skip non-productquickview layout related fields
            var skipFields = [
                'agl_metabox_nonce',
                '_wp_http_referer',
                '_pseudo',
                'shopg_productquickview_layouts_nonce',
                'shopg_shop_layoutid',
                'layout_name',
                'layout_template'
            ];
         ...
         * e.g., "shopg_productquickview_settings_template1[tab][field]" -> {shopg_productquickview_settings_template1: {tab: {field: value}}}
         */
        ...
        /**
         * Parse a field name like "shopg_productquickview_settings_template1[tab][field][0]" into an array of keys
         */
         ...
         */
        saveProductQuickviewLayoutData: function() {
            // Show loader
            ...
            var nonce = $('input[name="shopg_productquickview_layouts_nonce"]').val();

            // Prepare AJAX data
            var ajaxData = {
                action: 'save_shopg_productquickview_layoutdata',
                layout_name: layoutName,
                layout_template: layoutTemplate,
                shopg_productquickview_layoutid: layoutId,
                productquickview_nonce: nonce,
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        ShopGlutProductQuickviewLayout.dataConverter.showNotification('success', response.data.message || 'Order complete layout saved successfully!');
                        ...
                        // Show error message
                        ShopGlutProductQuickviewLayout.dataConverter.showNotification('error', response.data.message || response.data || 'Failed to save order complete layout.');
                    }
                },
                error: function(xhr) {
                    ...
                    ShopGlutProductQuickviewLayout.dataConverter.showNotification('error', errorMessage);
                },
                complete: function() {
                    // Hide loader
         ...
         */
        resetProductQuickviewLayoutSettings: function() {
            ...
            // Prepare AJAX data
            var ajaxData = {
                action: 'reset_shopg_productquickview_layout_settings',
                layout_id: layoutId,
                nonce: nonce
            };
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // Show success message
                        ShopGlutProductQuickviewLayout.dataConverter.showNotification('success', response.data.message || 'Settings reset to default successfully!');
                        ...
                        // Show error message
                        ShopGlutProductQuickviewLayout.dataConverter.showNotification('error', response.data.message || response.data || 'Failed to reset settings.');
                    }
                },
                error: function() {
                    ShopGlutProductQuickviewLayout.dataConverter.showNotification('error', 'Network error occurred while resetting settings.');
                },
                complete: function() {
                    // Hide loader
                    if (window.hideLoader && typeof window.hideLoader === 'function') {
    ...
    $(document).ready(function() {
        ...
        $('#productquickview-save-layout-button').on('click', function(e) {
            e.preventDefault();
            ShopGlutProductQuickviewLayout.dataConverter.saveProductQuickviewLayoutData();
        });

        // Bind reset button click
        $('#productquickview-reset-settings-button').on('click', function(e) {
            e.preventDefault();
            ShopGlutProductQuickviewLayout.dataConverter.resetProductQuickviewLayoutSettings();
        });

        // Quickview display selection AJAX - following select_products pattern
        $(".agl-field.agl-field-select_quickview_display .agl-fieldset").on("click", function () {
            if ($(".agl-field.agl-field-select_quickview_display .chosen-drop .chosen-results").length > 0) {

                // Validate required data exists
                if (!shopglut_admin_vars || !shopglut_admin_vars.ajax_url) {
                    console.warn('Missing required data for quickview display options request');
                    return;
                }
                ...
                if (!nonce) {
                    console.warn('Missing nonce for quickview display options request');
                    return;
                }

                // Show loading spinner
                $(".agl-field.agl-field-select_quickview_display .chosen-drop").append('<div class="loading-spinner"></div>');

                $.ajax({
                    url: shopglut_admin_vars.ajax_url || ajaxurl,
                    method: "POST",
                    data: {
                        action: "shopglut_get_quickview_display_options",
                        nonce: nonce,
                        layout_id: layoutId,
                    },
                    timeout: 15000, // 15 second timeout
                    success: function(response) {
                        console.log('Quickview display options response:', response);
                        ...
                        if (response.success && response.data) {
                            ...
                            $(".agl-field.agl-field-select_quickview_display .chosen-drop .chosen-results li").each(function() {
                                var itemText = $(this).text().trim();
                                // Remove the " (Used by another layout)" suffix for quickview
                    ...
                    error: function (jqXHR, textStatus, errorThrown) {
                        console.error("Failed to retrieve quickview display options:", {
                            status: textStatus,
                            error: errorThrown,
                        ...
                        if (jqXHR.status === 403) {
                            ShopGlutProductQuickviewLayout.dataConverter.showNotification('error', 'Access denied. Please refresh the page.');
                        }
                    },
                    complete: function () {
                        // Remove loading spinner
                        $(".agl-field.agl-field-select_quickview_display .chosen-drop .loading-spinner").remove();
                        $(".agl-field.agl-field-select_quickview_display .chosen-results").removeClass("filter-dis");
                    }
                });
            }
        });
