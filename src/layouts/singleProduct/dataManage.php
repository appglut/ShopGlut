...
class dataManage {
    ...
    public function shopglut_render_singleplayout_preview( $layout_id = 0 ) {
        ...
        if ( false === $cached_layout ) {
            ...
            $layout_data = $wpdb->get_row(
                $wpdb->prepare( "SELECT * FROM `{$wpdb->prefix}shopglut_single_product_layout` WHERE id = %d", $layout_id )
            );
        ...
        if ($is_pro_template) {
            ...
            $markup_class = 'Shopglut\\layouts\\singleProduct\\templates\\' . $template_name . '\\templateMarkup';
            $style_class = 'Shopglut\\layouts\\singleProduct\\templates\\' . $template_name . '\\templateStyle';
        } else {
            ...
            $markup_class = 'Shopglut\\layouts\\singleProduct\\templates\\' . $template_name . '\\' . $template_name . 'Markup';
            $style_class = 'Shopglut\\layouts\\singleProduct\\templates\\' . $template_name . '\\' . $template_name . 'Style';
        }
    ...
    public function shopglut_singleProduct_get_product_options() {
        ...
        $table_name = $wpdb->prefix . 'shopglut_single_product_layout';
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
        $results = $wpdb->get_results("SELECT id, layout_settings, layout_template FROM `{$wpdb->prefix}shopglut_single_product_layout`", ARRAY_A);
	...
	private function get_template_settings_keys($settings) {
		...
		// Check default settings only if it has overwrite enabled
		if (isset($settings['shopg_single_product_settings'])) {
			$default_settings = $settings['shopg_single_product_settings'];
			$has_global_overwrite = $default_settings['overwrite-all-products'] ?? false;
			$has_specific_overwrite = !empty($default_settings['overwrite-specific-products']);

			if ($has_global_overwrite || $has_specific_overwrite) {
				$keys[] = 'shopg_single_product_settings';
			}
		}

		// Find template-specific settings that have overwrite enabled
		foreach ($settings as $key => $value) {
			if (strpos($key, 'shopg_single_product_settings_template') === 0 && is_array($value)) {
				$has_global_overwrite = $value['overwrite-all-products'] ?? false;
				$has_specific_overwrite = !empty($value['overwrite-specific-products']);
	 ...
	 */
	private function get_template_priority($settings_key, $product_id, $specific_products, $overwrite_global) {
		$is_specific_match = in_array($product_id, $specific_products);
		$is_global_match = $overwrite_global;
		...
		if ($settings_key === 'shopg_single_product_settings') {
...
public function shopglut_save_single_product_layout() {
    
		// Clean nonce check
		if (!isset($_POST['shopg_singleproduct_layouts_nonce'])) {
			wp_send_json_error('No nonce provided');
			return;
		}
		
		if (!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['shopg_singleproduct_layouts_nonce'])), 'shopg_singleproduct_layouts')) {
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
		...
		if (isset($layout_settings[$template_settings_key])) {
			...
			// Check if "Overwrite All Products" is being enabled (value is '1', 1, or true)
			$overwrite_all = isset($template_settings['overwrite-all-products']) ? $template_settings['overwrite-all-products'] : false;

			if ($overwrite_all == '1' || $overwrite_all === 1 || $overwrite_all === true) {
				// Directly query DB to check if any other layout already has "Overwrite All Products" enabled
				...
				foreach ($existing_layouts as $existing_layout) {
					...
					$existing_template_key = 'shopg_singleproduct_settings_' . $existing_layout['layout_template'];
					...
					if (isset($existing_settings[$existing_template_key])) {
						...
						$existing_overwrite_all = isset($existing_template_settings['overwrite-all-products'])
							? $existing_template_settings['overwrite-all-products']
							: false;

						// Also check nested structure: single-product-settings -> overwrite-all-products
						if ((!$existing_overwrite_all || $existing_overwrite_all == '0' || $existing_overwrite_all === '') && isset($existing_template_settings['single-product-settings']['overwrite-all-products'])) {
							$existing_overwrite_all = $existing_template_settings['single-product-settings']['overwrite-all-products'];
						}

						// Check if existing layout has it enabled
						if ($existing_overwrite_all == '1' || $existing_overwrite_all === 1 || $existing_overwrite_all === true) {
				...
				if ($conflicting_layout) {
					// Another layout already has "Overwrite All Products" enabled, force disable it for this layout
					$layout_settings[$template_settings_key]['overwrite-all-products'] = '0';
				}
			}

			// Check if "Overwrite Specific Products" has products that are already used by a layout with "Overwrite All Products"
			$specific_products = $template_settings['overwrite-specific-products'] ?? [];
			if (!empty($specific_products)) {
				...
				foreach ($existing_layouts as $existing_layout) {
					...
					if (isset($existing_settings[$existing_template_key])) {
						$existing_template_settings = $existing_settings[$existing_template_key];
						$existing_overwrite_all = $existing_template_settings['overwrite-all-products'] ?? false;

						if ($existing_overwrite_all) {
							wp_send_json_error('Cannot select specific products because layout "' . esc_html($existing_layout['layout_name']) . '" (ID: ' . $existing_layout['id'] . ') already overwrites all products.');
							return;
						}
					}
				}
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
		...
		// Clear overwrite status cache when settings are saved
		wp_cache_delete('shopglut_overwrite_all_products_status', 'shopglut_layouts');
		wp_cache_delete('shopglut_single_product_layouts_all', ''); // Clear cache used in switcher.php
		wp_cache_delete("shopglut_layout_data_{$layout_id}", 'shopglut_layouts');
		wp_cache_delete("shopglut_layout_template_{$layout_id}", 'shopglut_layouts');
 ...
 */
public function shopglut_reset_single_product_layout() {
	// Clean nonce check
	if (!isset($_POST['shopg_singleproduct_layouts_nonce'])) {
		wp_send_json_error('No nonce provided');
		return;
	...
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
	$all_layouts = $wpdb->get_results("SELECT id, layout_name FROM `{$wpdb->prefix}shopglut_single_product_layout`");
	// error_log('All layouts in database: ' . print_r($all_layouts, true));

	// Check if layout exists
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
	$layout_exists = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM `{$wpdb->prefix}shopglut_single_product_layout` WHERE id = %d", $layout_id));
	...
	// Clear overwrite status cache when settings are reset
	wp_cache_delete('shopglut_overwrite_all_products_status', 'shopglut_layouts');
	wp_cache_delete("shopglut_layout_data_{$layout_id}", 'shopglut_layouts');
	wp_cache_delete("shopglut_layout_template_{$layout_id}", 'shopglut_layouts');
...
private function remove_empty_values($array) {
    ...
    foreach ($array as $key => $value) {
        if (is_array($value)) {
            // Special handling for overwrite-specific-products
            if ($key === 'overwrite-specific-products') {
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
    $table_name = $wpdb->prefix . 'shopglut_single_product_layout';
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
    $layout_options = unserialize($wpdb->get_var($wpdb->prepare("SELECT layout_settings FROM `{$wpdb->prefix}shopglut_single_product_layout` WHERE id = %d", $layout_id)));
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
    $layout_template = $wpdb->get_var($wpdb->prepare("SELECT layout_template FROM `{$wpdb->prefix}shopglut_single_product_layout` WHERE id = %d", $layout_id));

    $layout_settings = $layout_options['shopg_single_product_settings'] ?? array();
    $layout_settings_template = $layout_options['shopg_single_product_settings_'.$layout_template] ?? array();
