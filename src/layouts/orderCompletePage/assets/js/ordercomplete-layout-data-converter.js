...
(function($) {
    ...
    window.ShopGlutOrderCompleteLayout = window.ShopGlutOrderCompleteLayout || {};

    // Data converter module namespace
    ShopGlutOrderCompleteLayout.dataConverter = {
        ...
        shouldSkipField: function(name) {
            ...
            // Skip non-ordercomplete layout related fields
            var skipFields = [
                'agl_metabox_nonce',
                '_wp_http_referer',
                '_pseudo'
         ...
         * e.g., "shopg_ordercomplete_settings_template1[tab][field]" -> {shopg_ordercomplete_settings_template1: {tab: {field: value}}}
         */
         ...
         */
        saveOrderCompleteLayoutData: function() {
            // Show loader
            ...
            var nonce = $('input[name="shopg_ordercomplete_layouts_nonce"]').val();

            // Prepare AJAX data
            var ajaxData = {
                action: 'save_shopg_ordercomplete_layoutdata',
                layout_name: layoutName,
                layout_template: layoutTemplate,
                shopg_ordercomplete_layoutid: layoutId,
                ordercomplete_nonce: nonce,
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        ShopGlutOrderCompleteLayout.dataConverter.showNotification('success', response.data.message || 'Order complete layout saved successfully!');
                        ...
                        // Show error message
                        ShopGlutOrderCompleteLayout.dataConverter.showNotification('error', response.data.message || response.data || 'Failed to save order complete layout.');
                    }
                },
                error: function(xhr) {
                    ...
                    ShopGlutOrderCompleteLayout.dataConverter.showNotification('error', errorMessage);
                },
                complete: function() {
                    // Hide loader
         ...
         */
        resetOrderCompleteLayoutSettings: function() {
            ...
            // Prepare AJAX data
            var ajaxData = {
                action: 'reset_shopg_ordercomplete_layout_settings',
                layout_id: layoutId,
                nonce: nonce
            };
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // Show success message
                        ShopGlutOrderCompleteLayout.dataConverter.showNotification('success', response.data.message || 'Settings reset to default successfully!');
                        ...
                        // Show error message
                        ShopGlutOrderCompleteLayout.dataConverter.showNotification('error', response.data.message || response.data || 'Failed to reset settings.');
                    }
                },
                error: function() {
                    ShopGlutOrderCompleteLayout.dataConverter.showNotification('error', 'Network error occurred while resetting settings.');
                },
                complete: function() {
                    // Hide loader
                    if (window.hideLoader && typeof window.hideLoader === 'function') {
    ...
    $(document).ready(function() {
        ...
        $('#ordercompleteLayout-publishing-action #publish').on('click', function(e) {
            e.preventDefault();
            ShopGlutOrderCompleteLayout.dataConverter.saveOrderCompleteLayoutData();
        });

        // Bind reset button click
        $('#reset-settings-button').on('click', function(e) {
            e.preventDefault();
            ShopGlutOrderCompleteLayout.dataConverter.resetOrderCompleteLayoutSettings();
        });
    });
