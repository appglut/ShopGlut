...
class chooseTemplates {
	...
	public function __construct() {
		...
		add_action( 'admin_post_create_ordercomplete_page_layout', array( $this, 'handleCreateOrdercompleteLayout' ) );

	}


	public function loadOrdercompleteTemplates() {
		$tab_names = [
			'tab1' => [
						...
						<div class="shopg-template-preview" data-template="<?php echo esc_attr($layout_template); ?>">
							...
							<div class="template-content product-demo">
								<?php if ($layout_template === 'template1'): ?>
									...
									<div class="template-html-container">
										...
										<div class="html-preview-background">
											<?php $this->renderOrderCompleteTemplate1Preview(); ?>
										</div>

										<!-- Foreground: View Demo Button with Overlay -->
										<div class="html-preview-overlay">
											<button type="button" class="demo-view-btn" onclick="openHtmlDemoModal('<?php echo esc_attr($layout_template); ?>', 'order-complete')">
								...
								<?php else: ?>
									...
									<div class="template-image-container" onclick="openImageModal('<?php echo esc_attr($template_images[$layout_template]); ?>')">
										...
										<img src="<?php echo esc_url(SHOPGLUT_URL .'global-assets/images/ordercomplete_page-templates/'. $template_images[$layout_template]);// phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>"
							...
							<div class="template-footer">
								<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
									<input type="hidden" name="action" value="create_ordercomplete_page_layout">
									...
									$cache_key = 'shopglut_max_layout_id';
									$max_id = wp_cache_get( $cache_key );

									if ( false === $max_id ) {
										$max_id = $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQLPlaceholders.UnnecessaryPrepare -- Direct query required for MAX ID lookup, no user input involved
											"SELECT MAX(id) FROM {$wpdb->prefix}shopglut_ordercomplete_layouts"
										);
									...
									<?php wp_nonce_field('create_ordercomplete_page_layout_nonce', 'create_ordercomplete_page_layout_nonce'); ?>
		...
		<!-- HTML Demo Modal for Order Complete -->
		<div id="shopglut-ordercomplete-html-demo-modal" class="shopglut-template-modal-image-modal" style="display: none;">
			<div class="shopglut-template-modal-modal-content shopglut-html-demo-modal-content">
				<span class="shopglut-template-modal-close-modal" onclick="closeHtmlDemoModal('order-complete')">&times;</span>
				<div class="shopglut-template-modal-modal-body">
					<div class="html-demo-header">
						<h3 class="html-demo-header-title">
							...
							Order Complete Template One - Live Preview
						</h3>
						<p class="html-demo-header-subtitle">Explore the full interactive order complete page layout below</p>
					</div>
					<div id="orderCompleteHtmlDemoContent" class="html-demo-full-view">
						<?php $this->renderOrderCompleteTemplate1FullDemo(); ?>
					</div>
				</div>
			</div>
		</div>
	...
	/**
	 * Render Order Complete Template 1 Preview (scaled down for gallery view)
	 */
	private function renderOrderCompleteTemplate1Preview() {
		// Include the template markup class
		require_once SHOPGLUT_PATH . 'src/layouts/orderCompletePage/templates/template1/template1Markup.php';
		require_once SHOPGLUT_PATH . 'src/layouts/orderCompletePage/templates/template1/template1Style.php';

		$markup = new \Shopglut\layouts\orderCompletePage\templates\template1\template1Markup();
		$style = new \Shopglut\layouts\orderCompletePage\templates\template1\template1Style();
	...
	/**
	 * Render Order Complete Template 1 Full Demo (for modal view)
	 */
	private function renderOrderCompleteTemplate1FullDemo() {
		// Include the template markup class
		require_once SHOPGLUT_PATH . 'src/layouts/orderCompletePage/templates/template1/template1Markup.php';
		require_once SHOPGLUT_PATH . 'src/layouts/orderCompletePage/templates/template1/template1Style.php';
...
public function handleCreateOrdercompleteLayout() {

		
		if (
			!isset($_POST['create_ordercomplete_page_layout_nonce']) || 
			!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['create_ordercomplete_page_layout_nonce'])), 'create_ordercomplete_page_layout_nonce') ||
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
			...
			wp_cache_delete('shopglut_max_layout_id');
	
			// 8. Redirect on success
			$redirect_url = add_query_arg(
				array(
					'page' => 'shopglut_layouts',
					'editor' => 'ordercomplete',
